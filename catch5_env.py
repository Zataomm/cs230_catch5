...
class catch5():
    """ General environment to play the catch 5 game - to include 
        ...
        State: [BIDS(4),BID_SUIT(1),CARDS_IN_TRICK(4),CARDS_IN_HAND(9),
    ...
    def reset(self):
        ...
        for i in range(4):
            self.states[i][0,9:18]=np.sort(self.shuffled_deck[i*9:9*(i+1)])
    ...
    def legal_actions(self,state):
        ...
        if self.states[self.current_player][0,0]==0:  # still in bidding phase - first 8 are for bidding
            ...
            for i in range(3):
                if state[0,6+i] > 0:
                    lead_suit = int((state[0,6+i]-1)/13)
                    break
            cards = list(state[0,11++int(self.num_plays/4):18])
    ...
    def update_states(self,action):
        """ Update states based on current round and player acting.
            ...
            rounds.  
        """
        if self.states[self.current_player][0,0]==0: # action is a bid 
            for i in range(4):
                self.states[(self.current_player+i)%4][0,(4-i)%4]=action
        elif self.states[self.current_player][0,4] == 0: # action is selecting a suit 
            players_cards=[]                       
            for i in range(4):
                #set action
                self.states[(self.current_player+i)%4][0,4]=action
                #get cards for a redeal
                players_cards.append(list(self.states[(self.dealer+i)%4][0,9:18]))
            # deal new cards based on selected suits 
            new_hands=c5utils.dealPostBid(players_cards,self.shuffled_deck,action-1)
            for i in range(4):
                self.states[(self.dealer+i)%4][0,9:18]=new_hands[i]
            ...
            cards=list(self.states[self.current_player][0,9:18])
            cards.remove(action)
            cards.append(0)
            cards.sort()
            self.states[self.current_player][0,9:18]=cards
            # add card to everyones discard list - and to trick list 
            for i in range(4):
                j=(4-i)%4
                discards=list(self.states[(self.current_player+i)%4][0,18+6*j:24+6*j])
                discards[0]=action
                discards.sort()
                self.states[(self.current_player+i)%4][0,18+6*j:24+6*j]=discards
                #current trick list 
                self.states[(self.current_player+i)%4][0,5+j]=action
    ...
    def setup_next_round(self):
        ...
        if (self.num_plays%4) != 0: #not at end of round - keep going
            ...
            trick = list(self.states[self.current_player][0,5:9])
            scoop_suit=self.states[self.current_player][0,4]-1
            ...
            for i in range(4):
                self.states[i][0,5:9]=four_zeros
        elif self.num_plays == 4 and self.states[self.current_player][0,4] == 0:
            bidder,self.best_bid = c5utils.winning_bidder(list(self.states[self.current_player][0,0:4]))
